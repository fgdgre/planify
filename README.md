...
- Авторизація, захищені маршрути застосунку, підтвердження email, маршрут для password reset, privacy policy і terms pages.
...
| `/confirm` | Confirm Email | Обробка email confirmation link. |
| `/forgot-password` | Forgot Password | Відправка password reset email. |
| `/reset-password` | Reset Password | Маршрут для завершення reset flow. |
| `/app` | Dashboard | Захищена стартова сторінка застосунку. |
| `/app/calendar` | Calendar | Основна сторінка: unified calendar із Google та internal events. |
| `/app/notes` | Notes | Створення і перегляд нотаток, включно з event-linked notes. |
| `/app/settings` | Settings | Google connections і кольори календарних джерел. |
| `/privacy-policy` | Privacy Policy | Опис використання і зберігання Google Calendar data. |
...
app/
  ...
  pages/         Nuxt routes для landing, auth, legal pages і app
supabase/
...
Planify використовує Google Calendar data тільки для відображення, синхронізації та керування календарними подіями signed-in користувача. Застосунок зберігає лише дані, необхідні для роботи сервісу: metadata підключених акаунтів, календарі, синхронізовані події, нотатки та preferences користувача.
